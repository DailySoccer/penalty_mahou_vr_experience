...
public class AMTimeline : EditorWindow {
	...
	private float height_inspector_space = 20f;
	...
	void OnGUI() {
		...
		*/
		...
		if ( isPlayMode ) {
			if ( aData ) {
				float curFrame = timeToFrame( Time.time, (float)aData.getCurrentTake().frameRate );
				if ( curFrame < aData.getCurrentTake().endFrame )
					aData.getCurrentTake().selectedFrame = (int)curFrame;
			}
		}
		else if ( aData && (aData.getCurrentTake().selectedFrame == 0) ) {
			aData.getCurrentTake().selectedFrame = 1;
		}
		...
		Rect rectBtnExportPlay = new Rect(rectBtnOptions.x+rectBtnOptions.width+margin,rectBtnOptions.y,80f,rectBtnOptions.height);
		if(GUI.Button(rectBtnExportPlay,"Export Jugada", EditorStyles.toolbarButton)) {
			// EditorWindow.GetWindow (typeof (AMCodeView));
			MatchExport.ExportCurrentTake();
		}
		if(rectBtnExportPlay.Contains(e.mousePosition) && mouseOverElement == (int)ElementType.None) {
			mouseOverElement = (int)ElementType.Button;
		}
		Rect rectBtnExportAll = new Rect(rectBtnExportPlay.x+rectBtnExportPlay.width+margin,rectBtnExportPlay.y,80f,rectBtnExportPlay.height);
		if(GUI.Button(rectBtnExportAll,"Export All", EditorStyles.toolbarButton)) {
			// EditorWindow.GetWindow (typeof (AMCodeView));
			MatchExport.ExportAll();
		}
		if(rectBtnExportAll.Contains(e.mousePosition) && mouseOverElement == (int)ElementType.None) {
			mouseOverElement = (int)ElementType.Button;
		}
		#endregion
		#region take popup, change take or create new take
		Rect rectLabelCurrentTake = new Rect(rectBtnExportAll.x, rectBtnExportAll.y,rectBtnExportAll.width,rectBtnExportAll.height);
		if(!compact) {
			rectLabelCurrentTake = new Rect(rectBtnExportAll.x+rectBtnExportPlay.width+margin,rectBtnExportAll.y,80f,rectBtnExportAll.height);
	...
	string getInfoTextForAction(AMTrack _track, AMAction _action, bool brief, int clamped) {
		...
		if(_action is AMTranslationAction) {
			AMTranslationAction translation = _action as AMTranslationAction;
			string text = easeTypeNames[translation.easeType];
			if ( (translation.path != null) && (translation.path.Length > 1) ) {
				float len = AMTween.PathLength( translation.path );
				float tiempo = translation.getTime(aData.getCurrentTake().frameRate);
				if ( tiempo > 0 ) {
					text += " v: " + (len/tiempo).ToString("0.00");
				}
			}
			return text;
		#endregion
