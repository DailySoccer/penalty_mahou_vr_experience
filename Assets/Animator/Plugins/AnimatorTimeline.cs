...
public class AnimatorTimeline : MonoBehaviour {
	...
	public static void ParseJSON(string textAssetName) {
		TextAsset t = (TextAsset) Resources.Load(textAssetName, typeof(TextAsset) );
    ...
    {
        ParseJSONTake(JsonReader.Deserialize<JSONTake>(json));
    }

    public static void ParseJSONTake(JSONTake json)
    {
//        JSONTake j = JsonReader.Deserialize<JSONTake>(json);
        if (!parseJSONTake(json)) Debug.LogWarning("Animator: Error parsing JSON");
    }
